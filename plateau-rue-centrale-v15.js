...
function afficherNoeudRue(pays, noeudId) {
  ...
  // Personnes presentes dans cette scene de rue (meme affichage que dans les batiments)
  if (typeof renderPersonsList === 'function') {
    renderPersonsList(noeud.persons || []);
  }

  // Presence multijoueur : la rue centrale utilise le meme systeme que les batiments,
  // avec un buildingId synthetique 'rue-centrale' et le noeud comme roomId.
  ...
  if (typeof sbGetPresencesInRoom === 'function' && state.char?.name) {
    sbGetPresencesInRoom(pays, state.currentCity, 'rue-centrale', noeudId).then(presents => {
      if (rueCentraleNoeudActuel !== noeudId) return; // a change de scene entre temps
      const autresJoueurs = (presents || [])
        .filter(p => p.name !== state.char?.name)
        .map(p => ({ name: p.name, role: 'Joueur present dans la rue', rel: 'neutral', job: 'joueur_reel', estJoueurReel: true }));
      if (autresJoueurs.length > 0 && typeof renderPersonsList === 'function') {
        renderPersonsList([...autresJoueurs, ...(noeud.persons || [])]);
      }
    }).catch(() => {});
  }
