...
SOURCES = ''.join(en_expression(x) for x in [
    ...
    extraire(J, 'CAISSE_IMPRIMERIE', 'const'), extraire(J, 'CAISSE_REDACTION', 'const'),
    extraire(J, 'crediterCaisseRedaction'), extraire(J, 'encaisserRecetteRedaction'),
    ...
    extraire(C, 'SALAIRE_LOT_TRACTS', 'const'), extraire(C, 'lotsImprimablesIci'), extraire(C, 'produireLotsTracts'),
    ...
    # Le corps de sbSavePersonnage a ete scinde (chantier B, 13 septembre 2026) : la construction
    # du payload et la mise en file de l'ecriture vivent desormais dans deux fonctions dediees,
    # pour que la PREMIERE ecriture d'un personnage emprunte les memes 48 colonnes sans recopie.
    # Sans elles, le bac a sable les remplacerait par des bouchons muets et la chaine de delta
    # de POP ne serait plus testee du tout.
    extraire(lire('supabase.js'), 'sbPayloadPersonnage'), extraire(lire('supabase.js'), 'sbEnfilerEcriturePersonnage'),
]).replace('const ', 'var ')
...
var bouchons = {
  ...
  sbBatimentMouvementCaisse: function (p, v, b, sc, delta, stockCle, stock, stockMax, exigerExistant) {
    var cle = p + '/' + v + '/' + b;
    // FAIL-CLOSED : si la caisse demandee n'existe pas encore, la RPC refuse au lieu de la creer.
    if (exigerExistant === true && (!ETATS[cle] || !ETATS[cle][sc])) {
      return Promise.resolve({ ok: false, raison: 'caisse_absente' });
    }
...
%SOURCES%
  ...
  var scenarios = async function () {
    // ===== 1. Impression : 150 FR + 1 bois + 1 PA + 50 FR de salaire PAR LOT DE 10 =====
    ...
    avant = fonds() + imp().caisse;
    await imprimer('contre', 10);
    var t = bouchons.state.inventory.filter(function (i) { return i.type === 'tract'; })[0];
    verifier('P1. 10 tracts : 150 FR debites, 1 bois de l\'imprimerie, 1 PA, 50 FR de salaire, caisse +100 net',
      bouchons.state.liquide === 600 && bouchons.state.comptesBancaires.nationale.solde === 300 && bouchons.state.pa === 9
      && imp().caisse === 100 && imp().stockBois === 4 && t && t.quantite === 10 && t.tractType === 'contre'
      && fonds() + imp().caisse === avant,
      'liquide=' + bouchons.state.liquide + ' pa=' + bouchons.state.pa + ' caisse=' + imp().caisse + ' stock=' + imp().stockBois);
    verifier('P1 bis. le bois PERSONNEL du joueur n\'est jamais consomme', boisPerso() === 30, 'bois perso=' + boisPerso());
    verifier('P1 ter. le salaire est un vrai pouvoir d\'achat (liquide), pas un state.arg decoratif',
      bouchons.state.liquide === 700 - 150 + 50, 'liquide=' + bouchons.state.liquide);

    raz(); joueur({ liquide: 700, banque: 300 }); atelier(0, 5); avant = fonds() + imp().caisse;
    await imprimer('pour', 20);
    verifier('P2. 20 tracts : 300 FR, 2 bois, 2 PA, 100 FR de salaire, caisse +200 net',
      bouchons.state.liquide === 500 && bouchons.state.pa === 8 && imp().caisse === 200 && imp().stockBois === 3
      && boisPerso() === 30 && fonds() + imp().caisse === avant
      && bouchons.state.inventory.filter(function (i) { return i.type === 'tract'; })[0].quantite === 20,
      'liquide=' + bouchons.state.liquide + ' pa=' + bouchons.state.pa + ' caisse=' + imp().caisse + ' stock=' + imp().stockBois);

    raz(); joueur({ liquide: 2000, banque: 0 }); atelier(0, 10); avant = fonds() + imp().caisse;
    await imprimer('pour', 50);
    verifier('P3. 50 tracts : 750 FR, 5 bois, 5 PA, 250 FR de salaire, caisse +500 net',
      bouchons.state.liquide === 1500 && bouchons.state.pa === 5 && imp().caisse === 500 && imp().stockBois === 5
      && fonds() + imp().caisse === avant
      && bouchons.state.inventory.filter(function (i) { return i.type === 'tract'; })[0].quantite === 50,
      'liquide=' + bouchons.state.liquide + ' pa=' + bouchons.state.pa + ' caisse=' + imp().caisse + ' stock=' + imp().stockBois);

    raz(); joueur({ liquide: 5000, banque: 0 }); atelier(0, 10); bouchons.state.pa = 3; avant = fonds() + imp().caisse;
    await imprimer('pour', 50);
    verifier('P4. PA insuffisants (3 PA pour 5 lots) : refus avant tout debit, rien ne bouge',
      /PA insuffisants/.test(journal.toasts.join()) && bouchons.state.pa === 3 && bouchons.state.liquide === 5000
      && imp().caisse === 0 && imp().stockBois === 10 && !bouchons.state.inventory.some(function (i) { return i.type === 'tract'; })
      && fonds() + imp().caisse === avant, journal.toasts.join(' / '));

    raz(); joueur({ liquide: 60, banque: 50 }); bouchons.state.arg = 5000; atelier(0, 5); avant = fonds() + imp().caisse;
    await imprimer('pour', 10);
    verifier('P5. fonds reels insuffisants (110 < 150, meme avec arg=5000) : refus, bois et PA intacts',
    ...
    raz(); joueur({ liquide: 5000, banque: 0 }); atelier(0, 1); avant = fonds() + imp().caisse;
    await imprimer('pour', 20);
    verifier('P7. stock insuffisant pour la quantite (2 lots pour 1 bois) : refus total',
      /Stock de bois insuffisant/.test(journal.toasts.join()) && bouchons.state.pa === 10 && imp().stockBois === 1
      && !bouchons.state.inventory.some(function (i) { return i.type === 'tract'; }) && fonds() + imp().caisse === avant,
      journal.toasts.join(' / '));
    ...
    verifier('P8. double clic sur 1 seul bois : un seul lot produit, stock a 0, caisse +100, monnaie conservee',
      lotsDouble === 10 && imp().stockBois === 0 && imp().caisse === 100 && fonds() + imp().caisse === avant,
      'tracts=' + lotsDouble + ' stock=' + imp().stockBois + ' caisse=' + imp().caisse);

    raz(); joueur({ liquide: 5000, banque: 0 }); atelier(0, 10); bouchons.state.pa = 1; avant = fonds() + imp().caisse;
    await Promise.all([imprimer('pour', 10), imprimer('pour', 10)]);
    verifier('P9. concurrence sur 1 seul PA : un seul lot produit, PA jamais negatifs',
      bouchons.state.pa === 0 && imp().stockBois === 9
      && bouchons.state.inventory.filter(function (i) { return i.type === 'tract'; }).reduce(function (s, i) { return s + i.quantite; }, 0) === 10
      && fonds() + imp().caisse === avant,
      'pa=' + bouchons.state.pa + ' stock=' + imp().stockBois + ' caisse=' + imp().caisse);

    // ===== 1 bis. DEUX CAISSES DISTINCTES : imprimerie et redaction =====
    var red = function (p, v) { var e = ETATS[(p || 'republic') + '/' + (v || 'capitale') + '/la-tribune']; return (e && e.redaction) || {}; };
    var atelierEtRedaction = function (caisseAtelier, bois, caisseRedac) {
      atelier(caisseAtelier, bois);
      ETATS['republic/capitale/la-tribune'].redaction = { caisse: caisseRedac };
    };

    raz(); joueur({ liquide: 2000, banque: 0 }); atelierEtRedaction(0, 5, 300);
    await imprimer('pour', 10);
    verifier('X1. impression de tracts : credite la caisse IMPRIMERIE, la caisse JOURNAL ne bouge pas',
      imp().caisse === 100 && red().caisse === 300, 'imprimerie=' + imp().caisse + ' journal=' + red().caisse);

    raz(); joueur(); atelierEtRedaction(200, 0, 300); journal.saisies['vendre-bois-qte'] = '5';
    await confirmerVendreBoisImprimerie(0, 0);
    verifier('X2. achat de bois et salaire : sortent de la caisse IMPRIMERIE, la caisse JOURNAL ne bouge pas',
      Math.abs(imp().caisse - 172.5) < 1e-9 && imp().stockBois === 5 && red().caisse === 300,
      'imprimerie=' + imp().caisse + ' journal=' + red().caisse);

    raz(); joueur({ liquide: 2000, banque: 0 }); atelierEtRedaction(150, 5, 300);
    var avantX3 = fonds();
    var okX3 = await encaisserRecetteRedaction(800, 'le scandale');
    verifier('X3. recette editoriale : credite la caisse JOURNAL (+800), la caisse IMPRIMERIE ne bouge pas',
      okX3 === true && red().caisse === 1100 && imp().caisse === 150 && fonds() === avantX3,
      'journal=' + red().caisse + ' imprimerie=' + imp().caisse);

    raz(); joueur({ liquide: 2000, banque: 0 }); atelierEtRedaction(150, 5, 300);
    var okX4 = await encaisserRecetteRedaction(500, 'la corruption');
    verifier('X4. corruption reussie : meme caisse JOURNAL, jamais celle de l\'atelier',
      okX4 === true && red().caisse === 800 && imp().caisse === 150, 'journal=' + red().caisse);

    // Journal sans caisse editoriale initialisee : fail-closed, l'argent est rendu au joueur.
    raz(); joueur({ liquide: 2000, banque: 0 }); atelier(150, 5);
    var avantX5 = fonds();
    var okX5 = await encaisserRecetteRedaction(800, 'le scandale');
    verifier('X5. journal sans caisse editoriale : refus fail-closed, argent rendu, aucune caisse creditee',
      okX5 === false && fonds() === avantX5 + 800 && imp().caisse === 150 && !ETATS['republic/capitale/la-tribune'].redaction,
      'fonds=' + fonds() + ' imprimerie=' + imp().caisse);

    // Journal etranger : la recette va dans SA caisse, jamais dans celle de La Tribune de Republia.
    raz(); joueur({ pays: 'narco', liquide: 2000, banque: 0 });
    ETATS = { 'republic/capitale/la-tribune': { imprimerie: { caisse: 150, stockBois: 5 }, redaction: { caisse: 300 } },
              'narco/capitale/la-tribune': { imprimerie: { caisse: 200, stockBois: 0 }, redaction: { caisse: 0 } } };
    var okX6 = await encaisserRecetteRedaction(800, 'le scandale');
    verifier('X6. operation dans un journal etranger : credite SA caisse, jamais celle de Republia',
      okX6 === true && ETATS['narco/capitale/la-tribune'].redaction.caisse === 800
      && ETATS['republic/capitale/la-tribune'].redaction.caisse === 300,
      'narco=' + ETATS['narco/capitale/la-tribune'].redaction.caisse + ' republic=' + ETATS['republic/capitale/la-tribune'].redaction.caisse);
    ...
    verifier('F1. Luthecia : une seule entree « Imprimer des tracts », plus de bouton electoral ni calomnieux separe',
      ...
      && lut.indexOf('vendre_bois_imprimerie') >= 0,
      JSON.stringify(lut));
    // SEPARATION JOURNAL / IMPRIMERIE : l'accueil ne porte que l'atelier et le commerce.
    var redac = BUILDINGS['la-tribune'].rooms.redaction.orders.map(function (o) { return o.fn; });
    var ORDRES_JOURNAL = ['produire_fuite', 'fabriquer_scandale', 'interview', 'article', 'corrompre_journaliste', 'etouffer', 'consulter_archives_presse', 'deposer_petite_annonce'];
    var ORDRES_ATELIER = ['imprimer_tracts_electoraux', 'imprimer_tracts_calomnieux', 'imprimer_tracts_choix', 'vendre_bois_imprimerie'];
    // La petite annonce parait dans le JOURNAL : elle appartient a la redaction, pas a l'atelier.
    var redacLuthecia = (function () {
      var ctx = WORLD.republic.capitale.buildingContext['la-tribune'];
      var ov = (ctx && ctx.roomOverrides && ctx.roomOverrides.redaction) || {};
      var excl = ov.excludeOrders || [];
      return [].concat(BUILDINGS['la-tribune'].rooms.redaction.orders, ov.orders || [])
               .filter(function (o) { return excl.indexOf(o.fn) < 0; })
               .map(function (o) { return o.fn; });
    })();
    verifier('F1 bis bis. « Deposer une petite annonce » : presente en redaction, absente de l\'accueil',
      redacLuthecia.indexOf('deposer_petite_annonce') >= 0 && lut.indexOf('deposer_petite_annonce') < 0,
      'redaction=' + JSON.stringify(redacLuthecia) + ' accueil=' + JSON.stringify(lut));
    verifier('F1 ter. separation : aucun ordre journalistique a l\'accueil, aucun ordre d\'atelier en redaction',
      ORDRES_JOURNAL.every(function (f) { return lut.indexOf(f) < 0; })
      && ORDRES_ATELIER.every(function (f) { return redac.indexOf(f) < 0; })
      && ['produire_fuite', 'fabriquer_scandale', 'corrompre_journaliste', 'article', 'etouffer', 'interview', 'consulter_archives_presse'].every(function (f) { return redac.indexOf(f) >= 0; })
      && ORDRES_ATELIER.every(function (f) { return redacLuthecia.indexOf(f) < 0; }),
      'accueil=' + JSON.stringify(lut) + ' redaction=' + JSON.stringify(redac));
    ...
    verifier('F3. choix « electoraux » -> modal du moteur unique (POUR/CONTRE, quantite), 1 PA, 150 FR, bois de l\'imprimerie',
      ...
      && bouchons.state.pa === 9 && bouchons.state.liquide === 600 && imp().caisse === 100 && imp().stockBois === 4
    ...
    verifier('F4. choix « calomnieux » -> meme matiere que l\'electoral : 1 bois DE L\'IMPRIMERIE, 1 PA, 150 FR, detection',
      ...
      && bouchons.state.inventory.filter(function (i) { return i.stackKey === 'bois'; })[0].qty === 30
      && bouchons.state.liquide === 600 && tc && tc.quantite === 10 && tc.legal === false && imp().caisse === 100 && imp().stockBois === 4
    ...
    verifier('K1. calomnieux (vrai bouton Luthecia) : 1 PA + 150 + 1 bois DE L\'IMPRIMERIE, caisse +150, 10 tracts immediats, detection',
      titreCal === 'Imprimer des tracts calomnieux' && /150 FR/.test(corpsCal) && bouchons.state.pa === 9 && bouchons.state.liquide === 600 && bouchons.state.comptesBancaires.nationale.solde === 300
      && bouchons.state.inventory[0].qty === 30 && imp().caisse === 100 && imp().stockBois === 4 && lc && lc.quantite === 10 && lc.legal === false && lc.cible === 'Cible'
    ...
    await confirmerImprimerTractsCalomnieux(1, 150);
    verifier('K2. calomnieux liquide 100 + banque 300 : liquide 50 (salaire), banque 250, caisse +100 net, sans creation',
      bouchons.state.liquide === 50 && bouchons.state.comptesBancaires.nationale.solde === 250 && imp().caisse === 100 && fonds() + imp().caisse === avant,
    ...
    await confirmerImprimerTractsCalomnieux(1, 150);
    verifier('K4. Montrouge (LCI, salle Imprimerie) : meme prix, credite la caisse de l\'atelier de Montrouge', imp('republic', 'ville_b').caisse === 300 && bouchons.state.liquide === 600,
    ...
    verifier('K5. Port-Sainte-Marie (vrai bouton de l\'atelier Gutenberg) : 1 PA + 150 + 1 bois de Gutenberg, caisse +150, 10 tracts immediats',
      ordrePsm.cost === 150 && /150 FR/.test(corpsPsm) && bouchons.state.pa === 9 && bouchons.state.liquide === 600
      && bouchons.state.inventory[0].qty === 30 && psm().caisse === 300 && psm().stockBois === 4
    ...
    verifier('K6. electoraux (vrai bouton Luthecia) : 10 tracts immediatement en inventaire, aucune autre etape',
      bouchons.state.inventory.some(function (i) { return i.type === 'tract' && i.quantite === 10 && i.tractType === 'pour' && i.cible === 'Cible'; }) && imp().caisse === 100);
    ...
    verifier('D. boucle fermee : caisse 200 -> achat de 10 bois (-55) -> 20 tracts (+200 net, -2 bois) -> 345, monnaie conservee',
      e1.caisse === 145 && e1.stockBois === 10 && imp().caisse === 345 && imp().stockBois === 8
      && fonds() + imp().caisse === avant,
