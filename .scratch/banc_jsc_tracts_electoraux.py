...
var bouchons = {
  ...
  // Salaire de production : le joueur doit recevoir de l'argent REELLEMENT depensable (liquide).
  crediterFondsOrdinaires: function (m) {
    var st = bouchons.state;
    st.liquide = (st.liquide || 0) + m;
    st.arg = (st.arg || 0) + m;
    return m;
  },
...
%CRON%
  ...
  var scenarios = async function () {
    ...
    verifier('P1. Port-Sainte-Marie : 10 tracts = 150 FR, 1 PA, 1 bois DE GUTENBERG, 50 FR de salaire, caisse +100 net',
      ordrePsm.cost === 150 && bouchons.state.liquide === 600 && bouchons.state.comptesBancaires.nationale.solde === 300 && bouchons.state.pa === 9
      && bouchons.state.inventory[0].qty === 5 && psm().stockBois === 4 && psm().caisse === 300 && lotPsm && lotPsm.quantite === 10 && lotPsm.tractType === 'pour'
    ...
    await confirmerImprimerTractsElectoraux(ordrePsm.pa, ordrePsm.cost);
    verifier('P2. Port-Sainte-Marie : liquide 100 + banque 300 -> liquide 50 (salaire), banque 250, caisse +100 net',
      bouchons.state.liquide === 50 && bouchons.state.comptesBancaires.nationale.solde === 250 && psm().caisse === 300);
    raz(); joueurPsm(80, 20);
    await confirmerImprimerTractsElectoraux(ordrePsm.pa, ordrePsm.cost);
