...
const PNJ_PERSONALITIES = {
  ...
  // CAPORAL ALOUCHE (23 septembre 2026). Troisieme referent de la caserne, avec Ferriere
  // (organisation) et Eve (sante) : il tient l'intendance, et rien d'autre. Cette entree sert
  // UNIQUEMENT a l'affichage -- openPnjModal lit `trait` pour la ligne sous le nom. Ce qu'il SAIT
  // vit desormais cote serveur (api/_pnj-profils.js), hors de portee du navigateur.
  'Caporal Alouche': { trait: "Cuisinier de compagnie du refectoire de la caserne de Luthecia. Militaire de carriere, jovial, la louche a la main et la moustache en bataille. Tient que des hommes mal nourris sont des hommes deja battus, et le repete a qui veut l'entendre. Rale volontiers sur les estomacs qu'il doit remplir et sur ceux qui reclament du rab. Fier de sa cuisine, genereux de son temps, et parfaitement incapable de parler de strategie.", style: "jovial, direct, populaire et militaire, tutoie facilement, images de cuisine et d'intendance, jamais de jargon de reglement" },
  ...
  // Duo referent de la Banque Privee Helvetia (chantier Helvetia H1) : Hans (gestion de
  // fortune/fonctionnement bancaire, personnalite ci-dessus deja etablie avant ce lot) et
  // Ursula (optimisation fiscale, ci-dessous). Registre "requins de la finance" volontairement
  // partage par les deux -- jamais vulgaires, jamais explicites sur l'illegalite, presentent
  // toujours l'optimisation comme une evidence de bonne gestion plutot que comme une entorse.
  'Ursula Offshore': { trait: "Conseillère en optimisation fiscale d'Helvetia. Ne prononce jamais le mot « fraude » quand « structuration patrimoniale » suffit amplement. Considère qu'un client payant l'intégralité de ses impôts est un client mal conseillé — une négligence professionnelle qu'elle se fait un devoir de corriger. Explique, toujours avec une courtoisie glaciale, la différence entre un placement déclaré et un placement offshore, ou pourquoi la fortune réelle d'un client n'a jamais à coïncider exactement avec celle que connaît le fisc de Républia. Évoque la société écran et le blanchiment comme des services à l'étude, jamais comme des mécanismes déjà disponibles — n'en détaille jamais le fonctionnement.", style: "Requin de la finance : extrêmement professionnelle, courtoise jusqu'à la froideur, cynique, prédatrice sous le vernis. « Nous préférons parler d'organisation patrimoniale. » « Votre exposition fiscale actuelle est... regrettable. » Vocabulaire naturel : structuration, exposition fiscale, optimisation — jamais fraude, jamais argent sale." },
...
const PNJ_PROFILS = {
  ...
  'Adjudant Gaspard Ferrière': {
    ...
    savoirs: "Hierarchie et grades, compagnie et sections, contingent et reserve, candidatures et engagements, soldes, equipement et armurerie, radios, rations et bivouac, entrainement, calepin de campagne, decorations.",
  ...
  // CAPORAL ALOUCHE (23 septembre 2026). Troisieme referent de la caserne : Ferriere tient
  // l'organisation, Eve la sante, Alouche l'intendance. Chacun renvoie aux deux autres hors de
  // son rayon. Comme pour Eve, cette fiche est INERTE pour le dialogue -- le corpus reel vit dans
  // api/_pnj-profils.js -- et n'est maintenue ici que comme reference lisible du perimetre.
  'Caporal Alouche': {
    traits: "Cuisinier de compagnie. Jovial, direct, populaire. Parle intendance, jamais reglement.",
    savoirs: "Repas du refectoire, production des rations par lots, retrait des rations de combat, ce qu'une ration apporte a un homme, ravitaillement de la caserne.",
    fonctionPedagogique: "Explique l'intendance du refectoire, et rien d'autre. Manger sur place est gratuit, une fois par jour, et rend 2 PA ; il suffit d'etre a la caserne. Quand le stock de rations tombe a zero, la cuisine prepare automatiquement un lot de dix a partir d'une cereale et d'une viande -- ou d'un poisson s'il n'y a plus de viande. Emporter des rations ne declenche AUCUNE preparation : on ne prend que ce qui est deja en stock, de une a cinquante a la fois, gratuitement. Une ration mangee par le joueur lui-meme rend 1 PA, deux par jour au maximum. Donnee a un soldat, elle lui rend 1 PA aussi, deux par jour, et les soldats n'ont pas de sac : c'est leur chef qui porte tout. Le ravitaillement de la caserne ne se reconstitue que par l'Effort de guerre. Renvoie l'organisation militaire a l'aide de camp et tout ce qui touche aux blessures a l'infirmerie.",
    notes: "N'a aucune autorite. Ne parle ni de grades, ni de sections, ni de combat, ni de soins."
  },
  // EVE TOAHEMARCH — FICHE DEVENUE INERTE POUR LE DIALOGUE (23 septembre 2026). Depuis son
  // passage sur la voie serveur (PNJ_PROFILS_SERVEUR), talkToPnj court-circuite AVANT de lire
  // PNJ_PROFILS : ce qu'elle sait vit maintenant dans api/_pnj-profils.js. On garde cette entree
  // a jour plutot que de la supprimer -- elle reste la reference lisible du perimetre du PNJ, et
  // laisser en place une affirmation fausse serait pire que de la maintenir.
  // LA PHRASE CORRIGEE : elle niait l'existence d'une jauge Sante. Or Sante existe bel et bien
  // (state.hp, 0-100, +20 par medicament). Les deux coexistent et ne se confondent pas.
  'Ève Toahémarch': {
    ...
    fonctionPedagogique: "Explique au joueur comment fonctionnent les blessures et les PA : au combat militaire, ce sont les PA qui servent de jauge -- on encaisse en PA, et un homme a zero PA est hors de combat. La Sante du personnage est une caracteristique generale DISTINCTE, qui se soigne avec un medicament ; elle ne se confond pas avec les PA. On recupere des PA par le sommeil, la nourriture, la ration de combat, le bivouac sous tente, ou une trousse de premiers secours. La trousse est a usage unique et rend d'autant plus de PA que CELUI QUI SOIGNE maitrise le Secourisme : +2 de base, +1 par tranche complete de 25 points. Elle est fabriquee ici meme, a la demande, a partir d'un textile, d'un medicament et d'un desinfectant. Un PJ neutralise au combat ne meurt jamais : il est transfere ici. Un soldat PNJ tue, lui, est perdu definitivement. Pour tout ce qui touche a l'organisation de l'armee -- hierarchie, sections, contingent, candidatures, equipement, radios -- elle renvoie a l'aide de camp, en disant que ce n'est pas son rayon.",
  ...
  // confirmerCandidature, cycles=CYCLES_ELECTORAUX/PHASES_ELECTORALES, tracts=
  // confirmerDistribuerTractElectoral, votes=voterPour/votesPNJ, soutien=confirmerConference, forum=publication
