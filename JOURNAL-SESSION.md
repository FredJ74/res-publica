...
## État du dépôt
- Toutes les modifications ont été committées et poussées sur `main`. Session close ici (limite de quota de Fred) — reprise prévue la prochaine fois sur les 2 bugs confirmés en fin de session (voir chantiers en attente).
- Session reprise après la clôture de 3h : mécanique "moyenne de groupe" conçue et déployée partout, `distribuer_tract` construit de zéro dessus, économie du bois de l'imprimerie La Tribune ajoutée en prérequis, quête carrière complétée (aiguillage + 3 mini-missions des référents), refonte complète du système de postes, main d'œuvre de production pilotée sur l'Armurerie, puis découverte et résolution d'un vrai bug d'infrastructure Supabase (RLS) qui bloquait silencieusement `batiments_etat` depuis le début (voir sections dédiées ci-dessous).
...
**À reprendre en premier — 2 bugs confirmés par test réel en toute fin de session :**
1. **Bouton `produire_arme` affiche "gratuit"** alors que la production coûte réellement 2 PA et rapporte réellement 100 FR — la mécanique fonctionne (confirmé par Fred en jeu), seul l'affichage du bouton (`doProduireArme`, label/coût affiché) n'a pas été mis à jour avec le nouveau tarif plat codé ce soir (`c0f9b02`). Vérifier `doOrder`/le générateur de bouton (`renderRoomActions`, `plateau-politique.js`) : `produire_arme` est déclaré `pa:0, cost:0` dans `data.js` (la vraie logique est entièrement gérée dans `confirmerProduction`, hors du chemin `doOrder` standard) — c'est probablement ce `pa:0, cost:0` qui produit l'affichage "gratuit" trompeur sur le bouton de la salle, à corriger sans casser le fonctionnement réel (peut-être un texte dédié plutôt que les champs `pa`/`cost` génériques, cohérents avec d'autres ordres à logique custom).
2. **`vendre_matiere_armurerie` confirmé cassé en jeu** ("Stock personnel insuffisant" alors que le bois/métal acheté à l'entrepôt est bien dans l'inventaire) : `confirmerVenteMatiere` ne reconnaît que la forme d'inventaire "récolte" (`type:'matiere_premiere'`), pas la forme empilable produite par un achat à l'entrepôt (`stackable`/`stackKey`) — déjà identifié par investigation avant le test, maintenant confirmé par un vrai essai. Comme Luthécia n'a aucune ressource récoltable localement, cet ordre est **actuellement inutilisable dans la capitale**. Corriger sur le modèle de `vendre_bois_imprimerie` (construit le 9 août pour exactement ce même problème, côté imprimerie).

**À reprendre ensuite :**
3. **Main d'œuvre de production — pilote Armurerie fait ce soir, reste à dupliquer** sur Usine Pharmaceutique/Pôle Tabac & Alcools/Raffinerie/Entrepôts Logistiques une fois validé en jeu (voir section dédiée plus haut).
4. **Gap découvert ce soir en refaisant les postes, volontairement pas traité** : Commandant et les 4 Directeurs (pharma, tabac/alcools, raffinerie, entrepôt) n'ont aucun repli PNJ dans la cascade d'auto-provisionnement (`PNJ_PAR_DEFAUT_POSTE`/`CASCADE_NATIONALE`, `api/cron-minuit.js`) — resteront vacants indéfiniment si aucun joueur ne postule, contrairement à Président/PM/6 ministères/Juge/Maire/Commissaire. Pré-existant depuis la construction de la cascade le 8 août, pas introduit ce soir.
5. **Maire Adjoint (×3 villes) disparu** avec le retrait de l'ancienne table `POSTES` ce soir — poste orphelin qui n'était lu nulle part ailleurs. À réintroduire explicitement dans `POSTES_NOMMES_EXCLUSIFS` si un vrai rôle est voulu.
