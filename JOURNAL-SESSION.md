...
1. **Nouveau chantier à concevoir (pas à coder) : détection douanière des objets prohibés.** Demandé par Fred le 9 août — la douane (`hall_douanes`, Centre Multimodal) devra un jour inspecter réellement l'inventaire du joueur (armes achetées illégalement, autres produits sensibles), confisquer ce qui est prohibé, et déclencher une procédure de justification au commissariat en cas de découverte. Touche 3 systèmes à la fois (inventaire, douane/`passer_douanes_aeroport`, justice existante — arrestations, plaintes, tribunal) — un vrai système à cadrer ensemble avant de le lancer, pas un simple ajout ponctuel. Actuellement, passer la douane ne vérifie que le statut Recherché du joueur, jamais le contenu de son inventaire.
