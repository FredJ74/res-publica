# Journal de session — Res Publica (13 août 2026)

## État du dépôt
- Toutes les modifications ont été committées et poussées sur `main`.
- **Action requise de Fred avant de tester** : 2 nouvelles tables Supabase (`indices_villes`, `investissements`) doivent être créées via le SQL editor — script donné en fin de session, RLS désactivée d'emblée (même schéma que `budgets_municipaux`/`prets`).
- Session consacrée à trois chantiers : (1) 9 audioguides de musée branchés sur le mécanisme générique construit en début de session, plus un bug de lecture en fond corrigé et un bug de cache navigateur découvert et corrigé (root cause du 2e bug remonté par Fred) ; (2) audit complet des indices du jeu (Phase 1) et mapping des 344 ordres de Luthécia sur la grille de difficulté commune (Phase 2), livrés en `AUDIT_INDICES.md` + CSV/XLSX ; (3) grand chantier "refonte des ordres" — la Doctrine V2 rédigée par Fred, auditée en détail contre le code existant puis codée en 6 étapes committées séparément. Voir sections dédiées ci-dessous.

## 🎙️ Audioguides du musée (mécanisme générique) + bug de cache navigateur
- **Chantier ouvert par Fred** : construire un lecteur audio pour la Salle des Présidents, mais comme un vrai mécanisme générique réutilisable pour n'importe quelle salle, pas un ordre codé en dur.
- **Vérification préalable demandée** : confirmer que la Salle des Présidents appartenait bien au Musée National de Republia et non au Musée de la Ville de Luthécia malgré le nom du fichier audio (`musee-ville-luthecia-audio-salle-presidents.mp3`) — confirmé dans le code (`salle_presidents` n'existe que dans `musee-national-republia`), le nom de fichier était juste une convention de dossier par ville, pas par bâtiment.
- **Construit** : nouveau champ `audioUrl` sur la définition d'une salle (`data.js`), au même titre que `imageUrl` déjà existant. Bouton "Écouter l'audioguide" ajouté automatiquement par `renderRoomActions` (`plateau-politique.js`) pour toute salle ayant ce champ rempli — pas de `room.orders`/`doOrder`, juste une injection de bouton générique. `ouvrirAudioguide()` réutilise la modale générique déjà existante (`#modal-postes`) pour afficher un simple `<audio controls>`.
- **9 salles branchées au total** : Salle des Présidents (musée national), Salle d'Honneur Militaire (musée de la ville — vérifié pour ne pas confondre avec la Salle d'Honneur Militaire *Nationale*), puis 6 salles du musée national d'un coup (artistes/sportifs, réussites économiques, plus grands criminels du pays, responsables d'organisations, grandes villes, trésor national — vérifiées une à une contre les salles homonymes du musée de la ville, aucune confusion malgré des noms très proches comme "organisations" ou "criminels" partagés par les deux musées).
- **Bug signalé par Fred en testant** : le son continuait de jouer en arrière-plan après fermeture de la modale ou changement de pièce/bâtiment. Corrigé (`2b3c79e`) : nouvelle fonction `arreterAudioguide()` qui pause **et décharge réellement** l'élément `<audio>` (`removeAttribute('src')` + `load()`, pas juste `pause()`), câblée sur le bouton de fermeture de la modale et sur `enterRoom()` (choke point commun à tout changement de pièce ET de bâtiment).
- **2e bug signalé** : le bouton n'apparaissait pas du tout sur Honneur Militaire alors qu'il fonctionnait pour Salle des Présidents. Vérification complète demandée par Fred avant toute correction : le champ `audioUrl` était pourtant bien présent, au bon endroit, code de rendu strictement identique aux deux salles. **Cause réelle trouvée** : `plateau.html` charge chaque script avec un numéro de version en cache-buster (`data.js?v=168`) — convention du dépôt (confirmée dans l'historique git) qui veut qu'on l'incrémente à chaque édition d'un fichier, jamais fait sur mes 3 commits précédents. Un navigateur ayant déjà mis en cache `data.js?v=168` après le premier test ne retéléchargeait jamais la version suivante. Corrigé (`f80321d`) : versions bumpées, **et bump systématique désormais appliqué à chaque commit touchant `data.js` dans la suite de la session** (v169, puis v170 pour les 6 audioguides du musée national).

## 📊 Audit des indices du jeu + mapping des 344 ordres (`AUDIT_INDICES.md`)
- **Phase 1, demandée avant tout code** : inventaire complet de tous les indices du jeu (ISN/IE/ID/IS/IP nationaux, `INDICES_VILLES` local) — définition, ce qui les modifie aujourd'hui, ce qu'ils affectent réellement en jeu, trous identifiés. Livré en `AUDIT_INDICES.md` (`8a75492`). Trouvailles principales : ID (Diplomatie) a des dizaines de sites d'écriture pour un seul consommateur mécanique réel ; le multiplicateur de détection lié à l'ISN est affiché mais jamais appliqué ; IP (Piété) n'avait aucun consommateur du tout ; `INDICES_VILLES` était un système fantôme (une seule ville peuplée, une seule écriture, zéro lecture) ; deux propriétés orphelines `.POP`/`.INF` écrites sur `INDICES_NATIONAUX` par erreur (confusion probable avec les ressources du joueur) ; doublons de code morts (`getMalusIllegal`/`getMultDetection`, `checkDetection` défini deux fois).
- **Phase 2** : les 344 ordres de Luthécia mappés sur la grille de difficulté commune (Automatique/Facile/Standard/Difficile/Très difficile/Exceptionnel/Désespéré), à partir du tableau interactif déjà construit. 78% des ordres en "Automatique" (suspect), 10 ordres à 0% de réussite réelle étiquetés "Désespéré" à tort, ~40 ordres à ±5% du palier le plus proche (bruit de calibration). Export CSV (`3661b2a`) puis XLSX mis en forme avec couleurs par statut d'audit (`a671cae`), livrés à Fred qui a ensuite construit sa propre doctrine de refonte (V2) à partir de ce matériau.
- **Base directe du chantier suivant** — voir section Doctrine V2 ci-dessous.

## ⚖️ Grand chantier "refonte des ordres" — Doctrine V2, 6 étapes codées
- **Contexte** : Fred a rédigé une doctrine complète (formules, indices territoriaux vs nationaux, catégorisation des jets) à partir de l'audit ci-dessus. Avant tout code, audit de compatibilité demandé sur 4 points précis, puis décisions finales prises point par point sur 3 cas concrets (Filature/Bénédiction/Squatteurs), Arrestation/Excommunication laissés en suspens pour une session future.
- **Trouvailles de l'audit de compatibilité, avant tout code** :
  - `getStatEffective()` (construit le 9 août) calcule déjà la moyenne de groupe que la doctrine appelle `CAR_groupe` — mais ~45 formules existantes la consomment comme une magnitude brute (`stat×2`, `stat/3`...), pas comme un bonus centré sur 13 (`2×(CAR-13)`). Les deux logiques ne peuvent pas cohabiter dans une même formule ; chaque formule migrée doit être réécrite, pas juste reconnectée.
  - Le système de prêt n'a **aucun jet de risque** aujourd'hui (ni l'ancien menu Travaux/Consommation, déjà automatique, ni le nouveau système compromis, qui utilise un seuil déterministe de solvabilité à 10% de liquide, pas un dé) — la doctrine "emprunter automatique" décrit déjà ce qui existe. Le mécanisme garantie/défaut/saisie qu'elle réclame est également déjà construit en intégralité (`preleverPretsBancairesServeur`).
  - "Excommunier un fidèle" affiche `successRate:75` mais n'a **aucun jet ni aucun effet réel** — routé vers un cas générique qui affiche juste un toast, sans lire le taux ni toucher IP/POP. Rien à "changer de comportement", il n'y avait pas de comportement.
  - L'ordre "Investir" ne faisait **strictement rien** mécaniquement (aucune entrée dans `ORDER_EFFECTS`, l'ancienne fonction `doInvestir()` à 500 FR fixes n'était même jamais appelée par le routeur).
- **Collision de nommage identifiée et corrigée avant de coder** : l'ancien code utilisait `IS` pour "Social" et `ISN` pour "Sécurité" ; la doctrine V2 redéfinit `IS` = Sécurité. Plusieurs formules existantes (Filature et sa famille : évasion de prison, enquête, chasse à l'homme, cambriolage, vol de matériel de chantier, vol de PJ, achat d'arme illégale) lisaient déjà l'ancien `.IS` (Social) alors que ce sont toutes des actions policières/criminelles qui auraient dû lire la Sécurité — corrigé sur les 8 sites en même temps, pas seulement Filature (validé par Fred après explication).
- **Etape 1** (`60aa647`) — infrastructure : table Supabase `indices_villes`, 3 villes de Republia (Luthécia/PSM/Montrouge), les 5 indices territoriaux (Sécurité/Économie/Social/Piété/Moral) deviennent **calculés à la volée** comme la moyenne des 3 villes plutôt que stockés indépendamment au national — conforme à la doctrine (§5). Repli symétrique (lecture ET écriture) sur l'ancien `INDICES_NATIONAUX` pour les 3 autres empires, comportement strictement inchangé pour eux. Diplomatie (ID) reste indépendante, hors de ce système. Tous les lecteurs nationaux existants basculés vers le calcul (taux de prêt, coût de naturalisation, score militaire, génération de PNJ sur terrain, tableaux de bord).
- **Etape 2** (`9513d6a`) — les ~10 sites "locaux mais lisant le national" identifiés dans l'audit basculés vers l'indice de la ville concernée, avec la correction de collision ci-dessus appliquée à la volée.
- **Etape 3** (`80e6934`) — Filature : `P = 50 + 2×(PER_groupe−13) − (PER_cible−13) + (Sécurité_ville−50)/5 + (INF_commissaire−INF_cible)×0.3`.
- **Etape 4** (`74acf8e`) — Bénédiction devient un vrai service religieux demandé au titulaire (PJ ou PNJ) du poste `grand_pretre`, via le lookup générique déjà existant (`getTitulaireActuel`). `P = 80 + 2×(CHA_religieux−13) + (Piété_ville−50)/5`. Bonus consommable +5 câblé sur les 9 formules déjà retouchées dans ce chantier (périmètre limité, à étendre famille par famille plus tard). Piège évité : la négociation squatteurs a un aperçu en direct qui recalcule le taux à chaque frappe clavier — consommer le bonus dans la formule partagée l'aurait brûlé avant qu'un ordre soit vraiment joué ; déplacé au point de résolution réelle uniquement.
- **Etape 5** (`5cd6e57`) — Squatteurs : `P = 50 + 2×(CHA_groupe−13) + (Social_ville−50)/5 + bonus_argent` (nouveau barème par paliers, plafond +20 à 4000 FR). L'exception "Crime+Syndicat → intimidation automatique" demandée par Fred n'a **pas** été construite : elle est déclarée dans les données de bonus d'organisation mais n'a jamais été câblée nulle part dans le code fonctionnel — laissée telle quelle, sur décision explicite de Fred, pour une session future.
- **Etape 6** (`7a9405e`) — Investir reconstruit de zéro : montant libre (min 500 FR, pas de max), 1 PA, un seul investissement actif par joueur, snapshot d'INT à la mise, résolution automatique par le cron de minuit à J+7 (même mécanisme que `pretDemande`/`attente_validation`), IE de la ville lue **en direct** à l'échéance (pas snapshot). Nouvelle table Supabase `investissements`.
- **Méthode de livraison des 6 commits** : chaque étape codée d'un coup en une seule session de travail (pas au fur et à mesure de 7 messages séparés), puis rejouée en 6 commits distincts dans l'ordre du plan pour permettre une revue/un revert indépendant de chacun — vérifié après coup par diff complet que le rejeu reproduit exactement, ligne pour ligne, le code déjà validé (aucune divergence).
- **Hors périmètre, laissé pour plus tard** : extension du bonus Bénédiction à l'ensemble des jets du jeu (aujourd'hui limité aux 9 formules de cette passe) ; construction de l'exception Crime+Syndicat sur les squatteurs ; migration des ~70 sites d'écriture restants qui touchent encore `INDICES_NATIONAUX` directement (deviennent temporairement inertes sur les indices territoriaux tant qu'ils ne sont pas migrés — documenté en commentaire à chaque site) ; Arrestation et Excommunication (formules déjà auditées, décisions non encore prises) ; construction de l'infrastructure ville pour les 3 autres empires.

## 📝 Chantiers en attente (mis à jour)

**Résolu depuis la dernière rédaction de cette liste :**
- ~~Maire Adjoint (×3 villes) disparu~~ → réintroduit dans `POSTES_NOMMES_EXCLUSIFS` avec ses propres prérogatives (voir sessions précédentes, avant le début de celle-ci).
- ~~Grand audit Ordres~~ → Phase 1 (indices) et Phase 2 (mapping difficulté des 344 ordres) livrées cette session, base du chantier Doctrine V2 ci-dessus. Reste : vérifier un par un les 318 handlers dédiés eux-mêmes (portée initialement prévue, pas encore faite — la Doctrine V2 avance par familles de formules plutôt que par revue exhaustive ligne à ligne).

**Nouveau, issu du chantier Doctrine V2 de cette session :**
1. **Arrestation et Excommunication** — formules déjà auditées en détail (voir section ci-dessus), décisions de Fred pas encore prises. Arrestation : la doctrine propose de rendre l'ordre automatique et de déplacer le seul jet réel sur la tentative de fuite de la cible (mécanique `tenterFuite()` déjà existante et proche de ce schéma). Excommunication : à construire entièrement, rien n'existe aujourd'hui au-delà du texte de l'ordre.
2. **Extension du bonus Bénédiction** à l'ensemble des jets du jeu — aujourd'hui limité aux 9 formules retouchées dans cette session (Filature, Squatteurs, évasion, enquête, chasse à l'homme, cambriolage, vol de matériel, vol PJ, achat d'arme illégale). Nécessite d'identifier et de câbler `consommerBonusBenediction()` sur chacune des formules à jet restantes, famille par famille.
3. **Exception Crime+Syndicat (squatteurs)** — intimidation automatique, jamais construite malgré sa description dans les données de bonus d'organisation. À construire si Fred le juge prioritaire.
4. **Migration des ~70 sites d'écriture nationale restants** — décrets, traités, campagnes nationales, corruption de fonctionnaire, etc. — vers l'indice de ville concerné, ou requalification explicite en action réellement nationale. Documentés en commentaire dans le code comme "temporairement inertes" sur les indices territoriaux tant que non migrés.
5. **Infrastructure `indices_villes` pour les 3 autres empires** (narco/soviet/khalija) — scope volontairement limité à Republia cette session.
6. **Revue exhaustive des 318 handlers d'ordres** (portée originale de l'audit, pas encore faite) — la Doctrine V2 avance par familles de formules identifiées plutôt que ligne à ligne ; à voir avec Fred si cette revue exhaustive reste utile en complément.
...
## État du dépôt
- Toutes les modifications ont été committées et poussées sur `main`.
- Session reprise après la clôture de 3h : mécanique "moyenne de groupe" conçue et déployée partout, `distribuer_tract` construit de zéro dessus, économie du bois de l'imprimerie La Tribune ajoutée en prérequis, quête carrière complétée (aiguillage + 3 mini-missions des référents), refonte complète du système de postes, main d'œuvre de production pilotée sur l'Armurerie, découverte et résolution d'un vrai bug d'infrastructure Supabase (RLS) qui bloquait silencieusement `batiments_etat` depuis le début, correction des 2 bugs confirmés par le test en jeu, 12 images branchées, une série de 6 points remontés par un nouveau test en jeu, peaufinage de l'UX Armurerie, 2 correctifs post-refonte électorale (reset de l'argent au F5, victoire d'Arnie rétablie), 2 ajustements au Notaire (rachat d'entreprise généralisé, renommage compromis), phase de compromis pour le rachat d'entreprise sur le modèle du terrain, fix de la clause prêt bancaire du compromis (terrain + adaptation entreprise) vérifié par test isolé, persistance du journal personnel, main d'œuvre de production dupliquée sur les 3 usines avec vrai stock physique de matières, reprise du chantier "priorité PJ" (cascade Commandant/Directeurs + fenêtre de gestion des candidatures), audit RLS systématique des 66 tables Supabase (36 corrigées), fix urgent de la nomination de poste jamais réellement appliquée, création du poste de Maire Adjoint, puis correction de la collision d'id de terrain découverte au passage (voir sections dédiées ci-dessous).

## 🗺️ Collision d'id de terrain corrigée (terrain-a-batir-3, 4 empires)
- **Ampleur exacte établie avant tout code** (demande de Fred) : `terrain-a-batir-3` était partagé par 2 villes dans **chacun des 4 empires** (même id → même ligne `terrains_etat`, aucun champ ville pour les distinguer) — Republic (capitale ↔ Montrouge), narco/soviet/khalija (ville_a ↔ ville_b, paire différente de Republic). Vérifié directement sur Supabase : seule la ligne `republic_terrain-a-batir-3` avait des données réelles (joueur Bomboclat, permis en instruction) — les 3 autres empires n'avaient jamais été touchés.
- **Corrigé** (`4e29ac4`) : Montrouge passe à `terrain-a-batir-6` (déjà libre dans Republic, déjà défini) ; narco/soviet/khalija passent leur `ville_b` à `terrain-a-batir-2` (déjà libre dans ces 3 empires). Choix vérifiés pour ne réutiliser que des ids déjà définis, sans créer de nouveau lot.
- **Aucune perte de données** : la ligne de Bomboclat n'a jamais été celle de Montrouge — `data.city:"capitale"`, gravé au moment de l'achat, confirme qu'elle a toujours représenté Luthécia. Rien à réinitialiser.
- **Effet de bord positif signalé** : le lot de Montrouge passe d'un palier "simple" (achat + permis) à un palier complet (+ compromis, construction, squatters/cadavre) — les lots réattribués aux 3 autres empires restent au même palier, aucun changement fonctionnel.
- **Détail trouvé en creusant** : `PLAN_LAYOUTS.ville_b` (minimap, `plateau-navigation.js`) est une table générique partagée par les 4 empires — avait déjà une entrée `terrain-a-batir-6` prête (Republic), mais aucune pour `terrain-a-batir-2` (narco/soviet/khalija) : ajoutée. `TERRAINS_PAR_VILLE.ville_b` (filtre des permis par ville, ajouté la veille pour le Maire Adjoint) mis à jour pour Republic.
- **Vérifié** : script de contrôle systématique confirmant zéro collision résiduelle dans les 4 empires après correction.

## 🏛️ Création du poste de Maire Adjoint (prérogatives propres, transfert complet)
- **Inventaire préalable demandé par Fred** avant tout code : 9 prérogatives exclusives du Maire recensées et décrites une par une, dont une incohérence trouvée (`acte_officiel_mairie` absent du template partagé PSM/Montrouge) et une asymétrie (`nommer_directeur_entrepot` sans `revoquer` correspondant, contrairement à commissaire).
- **Construit** (`f0c6e13`) : poste `maire_adjoint` (`POSTES_NOMMES_EXCLUSIFS`, nommé par le Maire, scope ville), nouvelle pièce "Bureau du Maire Adjoint" aux 3 villes. 4 prérogatives transférées (`traiter_demandes_permis`, `acte_officiel_mairie` — corrigé aux 3 villes au passage —, `financer_communal`, `gerer_candidature_directeur_entrepot` + `nommer_directeur_entrepot`, déplacés des Entrepôts Logistiques). Nouveau `revoquer_directeur_entrepot` symétrique, trivial à construire (`ouvrirRevoquerPosteNomme` déjà 100% générique).
- **Point technique découvert en creusant** : `directeur_entrepot.nommePar` a dû passer de `'maire'` à `'maire_adjoint'` pour que les candidatures/révocations arrivent à la bonne autorité — sans impact sur le remplissage PNJ automatique du cron, qui a sa propre boucle câblée en dur sur `estOccupe('maire', ville)`, indépendante de ce champ.
- **Désignation de l'Adjoint** : même mécanisme de candidature/fenêtre groupée que les Directeurs (`gerer_candidature_maire_adjoint` au Bureau du Maire) — pas de cohabitation automatique liée au score électoral, délibérément pas construite (réflexion pour plus tard).
- **Bug corrigé au passage** (demande explicite de Fred) : `doTraiterDemandesPermis` chargeait les permis de tout le pays au lieu de filtrer par ville — nouveau `TERRAINS_PAR_VILLE`.
- **Collision d'id découverte en corrigeant ce bug, documentée mais non corrigée** : `terrain-a-batir-3` est utilisé à la fois par Luthécia et Montrouge (même id, donc même ligne Supabase `terrains_etat`, qui n'a pas de champ ville) — chantier séparé, voir section dédiée si traité ce soir.

## 🚨 Fix urgent : les candidatures acceptées et les démissions forcées n'accordaient jamais rien
- **Découvert en cataloguant les tables pour l'audit RLS** : `accepterCandidaturePoste` (candidature acceptée par l'autorité) et la démission forcée par motion de censure contre le PM dépendaient de `sbDeposerNominationPoste`/`sbGetNominationsPosteEnAttente`/`sbMarquerNominationTraitee` — recherche exhaustive (tous fichiers, toutes casses) : **ces 3 fonctions n'ont jamais été définies nulle part**, seulement appelées derrière des gardes `typeof === 'function'` qui les rendaient no-op silencieux. Le mail/toast/journal disaient "devient X" mais rien n'était jamais réellement accordé — cassé depuis la refonte des postes du 9 août, et par héritage direct le bouton "Nommer" ajouté ce soir dans la fenêtre de gestion des candidatures.
- **Bonne nouvelle vérifiée** : la règle de priorité PJ et la nomination directe (`accepterNominationPosteNomme`, fonction distincte et autonome qui écrit directement `state.poste`) n'étaient **pas** affectées — seule l'acceptation d'une candidature déposée par un PJ (bottom-up) était cassée.
- **Corrigé** (`81587be`) : les deux écrivent désormais directement sur `personnages.poste` via Supabase au moment de l'action (délogement de l'ancien titulaire PJ/PNJ compris), plutôt que de différer à une prochaine connexion qui ne pouvait jamais se produire — le candidat/PM visé n'étant pas forcément connecté au moment où l'action est déclenchée, on ne peut pas toucher son `state` local, seulement sa fiche. `appliquerNominationPosteEnAttente()` retirée (devenue totalement morte), ainsi que son unique appel au chargement du personnage.

## 🔍 Audit RLS systématique — 66 tables testées, 36 corrigées
- **Méthode** : inventaire complet des tables via grep de tous les appels `sbGet/sbInsert/sbUpdate/sbDelete` du code (66 tables trouvées), puis test complet par table (INSERT taggé `ZZTEST-audit`, relecture, UPDATE, DELETE avec re-vérification — un DELETE bloqué par RLS renvoie 200+`[]` sans erreur, comme découvert cette nuit sur `entreprises`, donc pas fiable de se fier au seul code retour).
- **Résultat** : 28 tables saines, **36 tables avec un vrai problème RLS** (8 avec INSERT totalement bloqué — `detentions`, `etat_civil_deces`, `etats_urgence`, `jugements`, `niveaux_prison`, `objets_recus`, `terrains_historique_ventes`, `titulaires_pnj` — et 27 avec UPDATE et/ou DELETE bloqué, `salons_membres` inclus), 1 table inconclusante (`demandes_naturalisation`, schéma trop de colonnes `NOT NULL` non devinables sans accès SQL), et une découverte annexe : `faits_armes` (`supabase.js:1228`) **n'existe pas du tout** dans la base — code mort ou table jamais créée, pas un problème RLS.
- **Corrigé** : script `ALTER TABLE ... DISABLE ROW LEVEL SECURITY` pour les 36 tables, exécuté par Fred dans l'éditeur SQL Supabase (aucune raison de garder RLS actif avec une vraie policy — même situation que `batiments_etat` cette nuit, une seule clé anon partagée par tous les joueurs). Les 27 lignes de test `ZZTEST-audit` laissées par les tables à DELETE bloqué ont été nettoyées après coup (27/27, confirmant au passage que le script a bien débloqué les suppressions).
- **Reste ouvert** : `demandes_naturalisation` (statut RLS non déterminé) et `faits_armes` (table manquante) — pas traités ce soir, hors périmètre de l'audit RLS lui-même.

## 🏛️ Reprise du chantier "priorité PJ sur PNJ" (point 2 du backlog)
- **Vérification avant de coder** : la règle elle-même (une candidature d'un vrai joueur auprès d'une autorité PNJ générique est acceptée directement, sans mail) était en fait **déjà codée** pendant la refonte des postes du 9 août (`demanderNominationPoste`, `plateau-politique.js`) — contrairement à ce qu'indiquait le backlog. Tracé toute la chaîne (`getTitulaireActuel`, `sbGetTitulairePnj`, `accepterNominationPosteNomme`) sans trouver de bug : couvre déjà les 6 catégories citées (Directeurs, PM, Ministres, Juge, Commissaire, Commandant).
- **Ce qui manquait réellement** : Commandant et les 4 Directeurs n'avaient aucun repli PNJ automatique dans la cascade du cron (gap déjà noté le 9 août) — donc jamais l'occasion pour la règle de priorité PJ de s'appliquer à leur niveau, faute d'autorité PNJ à déloger. **Corrigé** (`f603fd9`) : Commandant + 3 directeurs d'usine rejoignent `CASCADE_NATIONALE` (nommés par `min_def`/`min_fin`, aucun changement d'autorité) ; Directeur d'Entrepôt rejoint la boucle par ville existante (comme Commissaire), un PNJ différent par ville. PNJ par défaut : "Commandant Tom Hawak" (nouveau), le reste réutilise les PNJ déjà en poste dans chaque bâtiment (Bernard Piluler, Fernand Cendrier, Gustave Baril, Marcel Silo, Yvon Paletier, Norbert Charton).
- **Nouvelle fenêtre de gestion des candidatures groupées** (`ouvrirGestionCandidatures`), accessible depuis le bureau de l'autorité (Ministre des Finances pour les 3 directeurs d'usine, Maire pour le Directeur d'Entrepôt, Ministre de la Défense pour le Commandant) sans se déplacer physiquement. Réutilise intégralement le système de candidature/mail existant (`accepterCandidaturePoste`, jamais modifiée) — juste une interface groupée au lieu d'un mail isolé par candidat. "Nommer" applique directement (nomination en attente, effective à la prochaine connexion du candidat, délogement automatique de l'ancien titulaire) ; "Convoquer à un entretien" est purement narratif, sans effet mécanique, le candidat reste dans la liste ensuite.

## 🏭 Main d'œuvre de production dupliquée sur les usines (avec stock physique de matières)
- **Demande** : aligner le mode PNJ automatique (`produireTransformateursQuotidien`) sur le même ratio que le travail des PJ (1 matière = 2 produits, au lieu de l'ancien `RATIO_TRANSFORMATION=2` qui faisait l'inverse), et construire ce travail PJ — qui n'existait pas encore (vérifié avant de coder : aucun ordre de production PJ pour les usines nulle part, contrairement à ce que la formulation initiale laissait presque supposer).
- **Salaire calibré pour ~10% de marge usine**, calculs montrés et validés avant code, basés sur `RESSOURCES_ECONOMIE.prixAchatFournisseur` (1 PA = 5 matières → 10 produits) : **84 FR** (médicaments), **55 FR** (alcool, arrondi depuis 55,5), **66 FR** (tabac), **70 FR** (carburant).
- **Nouvel ordre "Produire..."** par chaîne, placé dans la salle de production dédiée déjà vide de chaque usine (`salle_production`/`distillerie`/`manufacture_tabac`) — pas besoin de liste de choix comme `produire_arme`, la salle détermine déjà le produit.
- **Correction importante avant de committer** (remontée par Fred) : la première implémentation faisait acheter la matière directement sur la caisse de l'usine (comme le fait déjà, historiquement, le mode PNJ automatique) — pas de vrai stock physique, alors que l'intention d'origine était que la livraison quotidienne à l'entrepôt (`livrerEntrepotsQuotidien`) en redirige une partie vers le stock local de l'usine. Vérifié avant de proposer un plan : ce mécanisme de redirection **n'existait nulle part**, même en commentaire — construction de zéro, pas une correction.
- **Construit** (`194b1a9`) : nouveau champ `usine.stockMatieres` (mirroir de l'Armurerie). `livrerEntrepotsQuotidien` redirige 20% de chaque livraison concernée vers le stock local de l'usine, carvé sur le même flux (pas un volume supplémentaire), gratuit pour l'usine — si son stock est déjà plein, le surplus reste à l'entrepôt (pas de perte), demande explicite de Fred. Couvre les 3 villes avec leur propre matière (Luthécia/plantes, PSM/céréales+plantes, Montrouge/pétrole) — vérifié explicitement par grep croisé avant de committer. Les deux modes de production (PNJ automatique et PJ) consomment désormais ce stock au lieu d'acheter sur la caisse.
- **Volume du mode PNJ automatique réduit à 10%** de son niveau d'origine (`VOLUME_MATIERE_PAR_CHAINE_JOUR` 80 → 8, soit 16 produits/jour au lieu de 40) — devient un filet de sécurité, les 90% restants étant censés venir du travail rémunéré des joueurs.

## 📔 Persistance du journal personnel (120 dernières entrées, FIFO)
- **Diagnostic demandé avant tout code** : contrairement au bug `arg` (donnée bien persistée mais écrasée par une resynchronisation manquante), `addJournalEntry()` (`plateau-core.js`) n'écrivait strictement **nulle part** en dehors du DOM (`#journal`, simple `<div>` vide dans `plateau.html`) — aucune sauvegarde à corriger, un vrai système à construire. Seule `addExternalEvent()` (événements partagés) avait déjà une persistance (Supabase `evenements_globaux` + cache "déjà vus"), non concernée par ce chantier.
- **Construit** (`63c0544`) : `state.char.journal` (tableau, 120 dernières entrées, `unshift` + purge FIFO à l'écriture) — délibérément **pas** de miroir `state.journal` séparé, pour ne pas réintroduire la classe de bug tout juste corrigée sur `arg` (valeur correcte oubliée lors d'une resynchronisation). Nouvelle colonne `journal` (jsonb) sur `personnages`, ajoutée par Fred sur le dashboard. `restaurerJournal()` réinjecte les entrées dans `#journal` aux deux points de restauration déjà existants de `loadCharacter()` (localStorage rapide, puis fusion Supabase) — même architecture double que le reste de l'état du personnage, rien de nouveau inventé.
...
2. ~~Main d'œuvre de production — pilote Armurerie fait le 9 août, reste à dupliquer~~ → **fait le 10 août sur Usine Pharmaceutique/Pôle Tabac & Alcools/Raffinerie** (voir section dédiée plus haut), avec stock physique de matières alimenté par redirection des livraisons entrepôt. **Entrepôts Logistiques non concernés** : modèle différent (directeur nommé, pas de production ouvrière).
3. ~~Gap découvert le 9 août en refaisant les postes~~ → **fait le 10 août** : Commandant et les 4 Directeurs (pharma, tabac/alcools, raffinerie, entrepôt) rejoignent la cascade d'auto-provisionnement (voir section dédiée plus haut), plus nouvelle fenêtre de gestion groupée des candidatures.
...
16. ~~Audit RLS systématique de toutes les tables Supabase~~ → **fait le 10 août**, 66 tables testées, 36 corrigées (voir section dédiée plus haut). Reste ouvert : `demandes_naturalisation` (statut RLS non déterminé, schéma trop complexe pour un payload de test sans accès SQL) et `faits_armes` (table manquante en base, pas un problème RLS — code mort ou jamais créée, à investiguer séparément).
