...
function obtenirNoeudRueCentraleMemorise(pays, ville, noeudDepartParDefaut) {
  return rueCentraleDerniersNoeuds[pays + '|' + ville] || noeudDepartParDefaut;
}
...
function afficherNoeudRue(pays, noeudId) {
  ...
  if (typeof state !== 'undefined' && state.currentCity) {
    rueCentraleDerniersNoeuds[pays + '|' + state.currentCity] = noeudId;
  }
  ...
  // Clones recadres par zone : chacun affiche la MEME image mais recadree (clip-path) sur
  // la tranche horizontale du batiment concerne. Au repos, invisible visuellement car
  // parfaitement aligne avec l'image de dessous. Au survol, seul ce clone grossit (centre
  // sur lui-meme), donnant l'impression que seul ce batiment s'agrandit — pas toute la rue.
  noeud.zones.forEach((z, i) => {
    const clipGauche = z.xPct[0];
    const clipDroite = 100 - z.xPct[1];
    html += '<img class="rc-clone" id="rc-clone-' + i + '" src="' + noeud.image + '" ' +
      'style="clip-path: inset(0 ' + clipDroite + '% 0 ' + clipGauche + '%); transform-origin: ' +
      ((z.xPct[0] + z.xPct[1]) / 2) + '% 50%;">';
  });
  ...
  noeud.zones.forEach((z, i) => {
    ...
    // Zoom localise : seul le clone recadre de CE batiment grossit au survol.
    div.addEventListener('mouseenter', () => {
      document.getElementById('rc-clone-' + i)?.classList.add('rc-clone-zoom');
    });
    div.addEventListener('mouseleave', () => {
      document.getElementById('rc-clone-' + i)?.classList.remove('rc-clone-zoom');
    });
